...
final class BoardCommentCardTool implements Tool
{
    ...
    private function writeRefusal(RequestException $e, int $cardId, string $agentName): \Throwable
    {
        ...
        return new ToolRefusalException(match ($status) {
            ...
            422 => "board_comment_card: the board REJECTED the comment on card {$cardId} (422), so nothing was written, and re-sending the same call unchanged will be refused the same way. The bridge's own length check passed before it sent: the comment, attribution line included, is within ".KanbanFieldLimits::COMMENT_MAX.' characters, so the board refused something other than that bound. Report it to your operator. '.BoardCallRefusal::boardReason($e),
        });
    }
}
