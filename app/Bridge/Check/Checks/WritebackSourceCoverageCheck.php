 ...
 * `source`, which the kanban derives from a card's `payload.repo` (when it names an
 * `owner/repo`), else a GitHub `pr_url` / `issue_url` / `html_url`, else its top-level
 * `external_link` ({@see ExternalReferenceNormalizer::sourceFor}). There a dl_number card
 * with none of those (source=null), or one whose derived owner/repo matches no repo mapped
 * to that board, is EXCLUDED by the lookup and silently never self-moves — indistinguishable from a
