 ...
 *                                                  // optional (card#6348) — the board's priority-lane stage ids; a lane-model-governed
 *                                                  // coord card is created in the stage its `stage:*` label declares, `later` when it
 *                                                  // declares none. Must carry `later`. Absent ⇒ every coord card lands in coord_card_stage_id
...
final class WritebackConfig
{
    ...
    public static function load(string $configDir): ?self
    {
        ...
        foreach ($rawMappings as $repo => $m) {
            ...
            // The board's PRIORITY-LANE stage ids (card#6348 / DL-286), keyed by the lane an
