...
final class ChannelSnapshotProbe
{
    ...
    public static function probe(?string $serverPath, string $bundledDir): array
    {
        ...
        // that is expected to be a symlink, needs its own guard.
    ...
    private static function versionLeg(string $deployedDir, string $bundledDir): array
    {
        ...
        if (self::compareVersions($deployed['version'], $bundled['version']) < 0) {
     ...
     *
     * The probe deliberately does NOT compare the deployment against this checkout's
     * file set, and does not read any file's CONTENT. A snapshot taken at an older
     * tag is legitimately missing files the checkout has since added, so a
     * completeness verdict reddens a stale-but-complete deployment — and it would
     * change nothing an operator does, because the STALE warn's remediation
     * (re-copy the whole directory + `npm ci`) already covers incompleteness. See
     * DL-229.
     *
     ...
     * directory (once, covering every stat both legs make — all of them are DIRECT
     * children of it, so none re-guards). The one deliberately unguarded stat is
     * this CHECKOUT's own bundled `package.json` ({@see self::versionLeg()}), which
     * lands on its own accurate, non-destructive WARN.
     *
