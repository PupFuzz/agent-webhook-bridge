...
| `app/Bridge/Check/Checks/*` | The individual checks, one class per leg: the install dirs, database reachability and the crosstalk guard, inbox surfacing, the per-agent classifier/secret/channel legs, the writeback cluster, board tools, the event-consumer leg, and the github webhook-subscription leg (card#9150 / DL-368 — the one leg that asks GITHUB whether a declared subscription still has a live hook here) |
