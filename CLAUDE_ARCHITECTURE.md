...
| `database/migrations/..._create_board_tools_client_calls_table.php` | `board_tools_client_calls`: one row per agent recording that seat's LAST SUCCESSFUL board-tools call and its transport (DL-313) — the seat's self-report that its CLIENT half works, which the bridge cannot observe any other way (it may not read the seat's own files). Written at `BoardToolDispatcher`'s success point, best-effort; read by `bridge:check`'s `board_tools.client_half` leg |
