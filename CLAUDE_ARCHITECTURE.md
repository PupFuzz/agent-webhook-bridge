...
| `app/Bridge/Support/ReceiverUrl.php` | The single shared receiver-URL shape `<base>/<provider>?b=<scope>` **and the EXACT-match predicate over it** — composed by `bridge:provision` to register a kanban subscription and to stay idempotent, and by `bridge:check` to ask whether a github repo still carries a hook delivering here (DL-368). Two copies would let one command adopt a subscription the other reports missing |
...
| `bridge:check` (`CheckCommand`) | Validate the install: config dir, DB connectivity, agent YAMLs parse, install-guard. Its legs are registered checks — see § The check registry. It also ends with a **NEXT STEPS** block (DL-352) that is deliberately NOT a check: it reports no verdict, reaches no disposition, is absent from the registered count, and cannot move the exit code — it derives the one command to run next per agent whose board-tools enablement is incomplete, from findings the run already produced |
