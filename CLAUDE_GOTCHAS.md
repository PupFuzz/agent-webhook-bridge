...
- a key the defaults **do not** have (say `https://api.github.com/*`) lands **first**, ahead of the `'*'` catch-all — which is exactly what makes it reachable at all;
- a key the defaults **already have** takes the caller's value and **is promoted to the front along with it**. It does *not* stay in the defaults' slot — `array_merge($defaults, $stubs)` is the form that would leave it in place.

That promotion is the same knife's other edge, and it is why an override must stay **narrowly scoped**: a caller overriding a *broad* pattern jumps ahead of narrower defaults it previously sat behind and silently kills them. Measured — a caller passing `'*'` promotes it above the `'*/tasks/search.json*'` default, and that default then never answers. Override the specific URLs under test, never `'*'`; a blanket stub also answers unrelated legs, so the fixture ends up carrying a second, unrelated diagnosis.

`array_merge($defaults, $stubs)` and `[...$defaults, ...$stubs]` are not substitutes: for a *new* key both append it **last**, i.e. behind the catch-all, where it never answers — while for an *overlapping* key they give the right value, so a stub set that only ever overrides existing keys masks the difference until someone adds a new pattern. Writing the operands the other way round (`$defaults + $stubs`) is wrong both ways: a new key goes last, and an overlapping key silently keeps the **default** value.
