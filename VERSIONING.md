...
10. Open the release PR `release/v<version>` → `main` (no ask to open; the user-merge gate in step 12 is the control point). **CRITICAL: the PR head must be the `release/v<version>` branch, NOT `dev` directly.** GitHub's "Automatically delete head branches" repo setting auto-deletes whichever branch is the merged PR's head — if you set head=`dev`, `dev` gets deleted when the user merges. Repo settings can't reliably exclude `dev` on the free plan (branch protection rules require Pro for private repos), so the discipline lives in the branch-naming convention.
    ...
    **Card coverage — the body reports it only when it MEASURED it, so read the body for which.** The release PR body is generated by the toolkit's `release-pr-body`, which dry-runs `promote-released-cards` over the release range and renders a `## Card coverage` section naming every shipped ref that has no tracking card on the board `.release-pr.json`'s `.promote` block names — the same correlation [`release-promote-cards.yml`](.github/workflows/release-promote-cards.yml) runs for real on the main-push, but early enough that a missing card is still a board edit rather than post-tag surgery. **The section renders only when the check could actually measure** (the board token, `.release-pr.json`'s `.promote` block, and the promote tool all reachable), and it renders **no placeholder** when it could not. So the body distinguishes four states, and only the first is a pass:

    | What the body carries | What it means |
    | --- | --- |
    | `## Card coverage` → *All shipped refs have a tracking card* (or *No shipped refs in range*) | Measured, clean |
    | `## Card coverage` → ⚠ *Shipped refs with no tracking card: …* | Measured, and it found a gap — card them before the merge |
    | `## Card coverage` → *Coverage check could not run (…exited N)* | It ran and errored — a bad token or an unreadable board, not a verdict |
    | **No `## Card coverage` section at all** | **The check DID NOT RUN.** Not a pass |

    **When the section is absent, run `promote-released-cards --dry-run` over the release range yourself before the PR is merged.** With no coverage section in the body it is the only thing between this release and a card stranded in Shipped-to-Dev — the post-merge promote reports a cardless ref after the tag has landed, where the fix is board surgery instead of a commit (DL-165 / DL-172 are what that failure looks like from the other end). Reading the absent section as "nothing to report" is precisely the misread this step exists to prevent.
