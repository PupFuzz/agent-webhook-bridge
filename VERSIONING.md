...
**"The PR changes …" in both rows means the MERGE-BASE diff** — `git diff --name-only "$BASE...$HEAD"`, the files this branch changed since it forked — and **not** everything that differs between the base tip and the head. A branch not up to date with its base carries the base's own post-fork commits in that second range, which made both verdicts a function of what merged on `dev` while the branch was open; card#8441 ([DL-337](CLAUDE_DECISIONS.md)) is the correction, and because the merge-base set is contained in the old one it can only stop the gate firing, never start it. The three-dot range needs the merge base to be REACHABLE, which is the second reason that job's checkout pins `fetch-depth: 0`.
