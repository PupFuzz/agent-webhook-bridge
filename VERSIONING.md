...
**A branch cut BEFORE a release fold is a distinct failure with the opposite remedy, and the gate says which one it is (card#8339).** The fold in step 4 above renames `## [Unreleased]` to `## [X.Y.Z]` and opens a fresh empty one above it; a branch open across that keeps its entry under the heading git now reads as `[X.Y.Z]`, with no conflict and no author error, so it merges into a **released** section that then claims work that release did not ship. The `[Unreleased]`-entry assertion fires — correctly — and would once have answered *"add an [Unreleased] entry"*, which files a **second** entry and leaves the first corrupting the released section. It now reads the merged changelog (`actions/checkout` gives the gate `refs/pull/<n>/merge`), finds the released section carrying a line **this branch introduced**, and — **only when that section was cut while this branch was open, which it proves by requiring the heading to stand on the base and to be ABSENT at the branch's FORK POINT — the parent of its own oldest commit, not its merge-base, which a branch that merges the base absorbs the fold into** — names it: *"your entry is under `[X.Y.Z]` because the branch predates the fold — move it under `[Unreleased]`"*. A branch that merely edits an older released section, or that cuts the section itself, fails that predicate and gets the message the gate always printed. **This is a diagnostic only — the verdict is byte-for-byte what it was.** [DL-329](CLAUDE_DECISIONS.md) owns the predicate and names its residuals; this paragraph does not restate them.
