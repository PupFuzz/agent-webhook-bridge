...
class WritebackRefusalSignalCoverageTest extends TestCase
{
    ...
    private const DISPOSITIONED = [
        ...
        'Bridge/Tools/BoardScopedRow.php::forCard#1' => 'REPORT+REFUSE at every caller — each card-id write tool, through `BoardScopedRow::lookUp` — and the refusal is deliberately NON-DISCLOSING: re-establishes on the ROWS that the board-scoped `cardRowsOnBoard($boardId, $cardId)` search answered, because an unrecognised search term degrades to free text and still answers 200 (DL-326). No match → the calling tool\'s own refusal message, nothing written. ⭐ The site MOVED here at DL-372, when `board_take_card` became its second caller — one owner rather than two copies of a tenant-boundary predicate; `BoardCorrectCardTool::matchingRow` was REMOVED in that same change and its caller now reads this. ⚠ And the method is spelled `forCard`, not `match`: PHP tokenises `match` as `T_MATCH`, so a method by that name never opens a body for {@see \Tests\Support\SourceScan::sitesInApp} and every site inside one is attributed to `(file scope)`, sharing one ordinal with the whole file. Routing it through the primitive would mint a writeback alert with no repo AND report the card\'s actual board, collapsing the not-yours / not-mine / unreadable-board answers DL-326 Decision 9 keeps identical',
