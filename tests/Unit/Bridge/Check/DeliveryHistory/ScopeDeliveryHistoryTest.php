...
class ScopeDeliveryHistoryTest extends TestCase
{
    ...
    {
        $history = ScopeDeliveryHistory::fromAscendingTimestamps($this->every(6 * self::HOUR, 10 * self::DAY));
    ...
    public function test_a_long_record_with_fewer_gaps_than_the_minimum_cannot_derive(): void
    {
        ...
        $history = ScopeDeliveryHistory::fromAscendingTimestamps([self::T0, self::T0 + 8 * self::DAY]);

        $this->assertGreaterThanOrEqual(ScopeDeliveryHistory::MIN_SPAN_SECONDS, $history->span());
        $this->assertFalse($history->isDerivable());

        $witness = ScopeDeliveryHistory::fromAscendingTimestamps([self::T0, self::T0 + 4 * self::DAY, self::T0 + 8 * self::DAY]);
