...
`git pull` only touches the repo. A few things the install depends on are **copied or hand-derived from the shipped samples (`examples/` + `docs/customization.md`) at install time** and live OUTSIDE what the repo ships, so a pull can't update them and they drift silently from the refreshed references. After every update, reconcile each against its reference — **diff-and-port; these are operator-customized, never blind-overwrite.** The **launcher** doesn't trip `bridge:check` at all (it runs inside the Claude Code *session*, not the Laravel app), so the diff is its **only** drift signal — and a stale connector is exactly how a session comes up **deaf to live-wake** ([`CLAUDE_DECISIONS.md`](CLAUDE_DECISIONS.md) DL-154/DL-155). The **`.mjs` snapshot** is **partially** checked **once you declare `channel.server_path`** in the agent YAML (DL-229/DL-230): `bridge:check` then WARNs when the deployed `package.json` version is behind this checkout's, and FAILs when the path is dangling, when it names a file rather than the directory, when the directory holds no entry `.mjs` (nothing to launch), when it has no `node_modules` (the `cp -R` whose `npm ci` never happened — node dies on `ERR_MODULE_NOT_FOUND` at the next session start), or when its version EQUALS this checkout's while the directory is missing a file the reference ships (DL-230 — the shape that motivates it is the cherry-picked deployment, which carries the current version stamp and reads healthy on every other leg, though the message states only the measurement and never how yours was assembled; gated on that equality, so a stale copy is never asked and a newer one is not measured against this checkout at all). Every one of those legs `stat`s the deployment, so when the bridge's OS user cannot traverse into the deployed directory they are all replaced by a single *not visible to this user* WARN naming that directory — run it as the agent's user if you need them to conclude. Undeclared, it reports *not validated* at severity **`unvalidated`** — counted in the run's closing tally, never `ok` (card 5170), so a green `bridge:check` is not evidence about the snapshot — and the diff is again your only signal — the bridge cannot infer the path (it may run as a different OS user and cannot read the agent's `.mcp.json`). **The diff still matters even when declared:** the check never reads a file's **content**, and it compares file sets in one direction and only at an equal version — so a hand-edited copy is invisible to it, as is a **stale** copy missing files (that population gets the STALE warn instead) and any extra file of your own. The version WARN catches a stale **snapshot**, not a hand-edited one (a local modification that never touched `version` still needs the diff). A custom classifier is the partial exception: `bridge:check` confirms it *loads* (FQCN resolves + implements `Classifier`) but **not** that it's *current* with the reference, so the diff is still the only signal that it's behind on an improvement (DL-158).
