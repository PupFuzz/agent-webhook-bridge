...
5. **The seat-tools pack** (only where one is staged — a PM's `OUTBOUND/<agent>/`, or a checkout seat's link-shape pack). A **copy-shape** pack is a snapshot of `bin/seat-pack.py`'s output and drifts like the channel-server copy: after the update, re-stage it from the release-pinned checkout (`python3 bin/seat-pack.py --out <coordination-repo>/OUTBOUND/<agent>/`) and commit it with its exec bits — [`docs/seat-tools.md`](docs/seat-tools.md) owns the command, the `100755` requirement and what the PM can and cannot see. To check without writing, regenerate into a temp dir and `diff -r` it against the staged `seat-tools/` and `channel-setup/`; a non-empty diff is a stale pack. A **link-shape** pack follows the checkout, so it needs a re-run only when `seat-tools.json` gains a tool. Unlike every item above, this one is regenerated rather than diff-and-ported: the pack is generated output, not an operator-customized copy.
