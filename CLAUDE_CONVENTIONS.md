...
⛔ **The branch's card must be the one the writeback SELECTS, and a `DL-<number>` is no substitute** (card#10031). The writeback reads ONE card token — the head ref's when it carries one, otherwise the title's **leftmost** — so on a `<type>/<id>-slug` branch a subject that cites another card before the trailing `(closes card#<id>)` moves the cited card. Cite other cards after your own token, or not at all. A DL may sit beside the card token; on its own it names no one card. `pr-title-lint.yml` § *Require the branch's card token for card-id branches* reds both, and says which card would have moved.
