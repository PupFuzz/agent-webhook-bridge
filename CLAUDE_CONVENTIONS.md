...
⛔ **PUT YOUR OWN CARD'S TOKEN FIRST, and a `DL-<number>` is not a substitute for it** (card#10031). The writeback SELECTS one card — the head ref's token when the ref carries one, otherwise the title's **LEFTMOST** token — so a title that cites another card *before* its own correlates to the cited one: the merge moves that card and yours never moves. Citing a second card is fine and common; it just goes **after** yours. `pr-title-lint` reds the wrong order now and names the card that would have moved instead. A bare `DL-<number>` stopped satisfying the gate in the same change: a DL resolves against the board, where it is bound to no one card, so it can carry the move to whichever card owns it. Keep the DL when it is relevant — it simply cannot stand in for the card token.

⛔ **A REVERT is the one PR this convention does NOT apply to, and it is exempt at both layers** (card#8306). GitHub composes a revert by quoting the original's title, so a revert of a PR written to this convention carries `(closes card#<id>)` **for work it undoes** — a claim its author never made. `pr-title-lint` exempts `revert-*` branches from demanding a closure, and the writeback refuses to read one out of the quoted span or out of GitHub's wrapped branch ref; [`docs/writeback.md`](docs/writeback.md) § *Mention vs closure* owns the ruling, the nesting case and the way to close a card ON a revert deliberately. Do **not** hand-edit a revert's title to satisfy the convention.

**The check.** [`.github/workflows/pr-title-lint.yml`](.github/workflows/pr-title-lint.yml) § *Require a closure claim on a card-correlating PR* reds any PR that correlates a card and claims closure on neither route, and renders the accepted spellings into its own failure output rather than asking you back here.

⭐ **A PR that cites a card it deliberately does not finish declares that with the literal `[no-close]` in the title, and BOTH layers honour it** (card#8294 for CI; card#8344 / DL-327 for the runtime). The context-PR case must stay possible or the gate becomes an over-promotion machine. Since card#8344 the marker is no longer CI-only: `pr-title-lint` skips its closure demand **and** the writeback takes **neither** closure route on merge, so the card is correlated and stamped exactly as usual and simply not moved. ⛔ It can only ever **withhold** a move — never cause one — which is why it is not the accept-surface DL-318 declined for the inverse `[close-anyway]` marker; it is also **independent of the card-side pin** (`block_reason` / `no-automove`): both hold, either alone withholds, neither overrides the other. It is a **literal**, not a grammar — `no close intended` is prose and closes the card. [`docs/writeback.md`](docs/writeback.md) § *Mention vs closure* owns the rule.
...
- **A runbook never resolves a secret VALUE onto a readable surface.** stdout, an argv, a log, or the shell history a command-line literal lands in — the surface is the rule, because the next instrument that resolves a secret is never on anyone's list of instruments (DL-321). Any doc or script telling an operator to place, pass, test for or compare a secret uses the forms in [`docs/config-schema.md § Handling a secret VALUE`](docs/config-schema.md#handling-a-secret-value-not-just-its-file), which owns them; do not restate them at the call site, link to them. ⚠ The reader may be an AI agent EXECUTING the block, so "the operator needs to see it" does not license an `echo` — write the value to a `0600` file and tell them to open it themselves. ⚠ **CI backstops this over markdown shell fences only** (`bin/check-doc-secret-fences.py`, DL-324) — a net under the rule, not the rule: it never reads prose, so quote a bad form in inline backticks when you need to explain one, and never read its green as evidence you followed this line.
...
- **HTTPS on the secret-bearing endpoints.** A provider API base URL carries the writeback bearer and, at provision time, the freshly-minted webhook HMAC secret, so it is validated with `UrlValidator::secureHttpUrl` — cleartext `http` is refused unless the host is loopback. The receiver base URL goes through `UrlValidator::httpUrl`, which enforces the URL shape and an http(s) scheme but **no** https floor. The idle-nudge base URL (`BRIDGE_IDLE_NUDGE_BASE_URL`) goes through `secureHttpUrl` too, for the same reason — it carries the fleet-read bearer. ⛔ **No count of the callers is kept in prose anywhere** (card#9528: the sentence here and `UrlValidator`'s own docblock both said *two* while three keys were being read); `command grep -rn 'UrlValidator::' app/` is the list. A URL-shaped value elsewhere is validated by whoever owns it: `channel.url` is shape-checked at its parse site in `AgentConfig` (non-empty, no whitespace; the loopback/SSRF gate is the `channel_push` handler's), and `alert_channel.url` defers to the sender's own `LocalhostUrl::assertValid`.
- **A config-value refusal that would break a working install goes at the DOOR, not in the shared validator.** `UrlValidator::httpUrl`/`secureHttpUrl` are asked at runtime as well as at setup, so a rule added to them stops installs that are working (card#9528: a userinfo character the HTTP client itself percent-encodes). Such a rule belongs on the `configDoor…` pair, which `bridge:check` and `bridge:provision` opt into by name — canon #3, a new need gets a dedicated path rather than a widened guard other callers rely on.
