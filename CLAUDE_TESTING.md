...
**Deriving the population, if you ever have to.** Never by `grep`: `grep`ping for `Http::fake` cannot bound it, because a class that calls `Http::fake()` and stubs some of its URLs escapes on the others. With the recorder in place, **turn the guard on, run the whole suite, and read the reds** — every refused request now fails its own test and the failure message names the URL. ⛔ **That was FALSE before the recorder, and by 96%**: a run reported 2 where the throw site reported 57. If you are ever measuring a tree where the recorder is absent or disabled, use the method **DL-303 Decision 2** records instead — record `<test>\t<uri>` at the point `StrayRequestException` is *constructed*, run the whole suite, and `sort -u` the file — and **run a control first** (drive one request you know is unstubbed and see it appear), because an instrument that records nothing and a clean suite produce the same empty file. Either way the count excludes whatever a test declared with `expectStrayRequest()` and the one allowance below — deliberately stated as a property and not a number, because a number here is the next thing to go stale — and says nothing at all about a catch-all test.
