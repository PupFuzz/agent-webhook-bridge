...
**⛔ An unstubbed request is blocked — by `Tests\TestCase`, not by `Http::fake()`.** The distinction is the whole entry. `Http::fake()` does NOT block a request no stub answers: `PendingRequest::buildStubHandler()` invokes every registered callback, and when none of them answers it falls through to the **live** handler (a bare `Http::fake()` or a `'*'` closure answers everything and hides this; the URL-pattern array form, the majority form in this suite, does not). `tests/TestCase.php` closes that by calling `Http::preventStrayRequests()` in `setUp()` (**DL-303**), so an unmatched request raises `StrayRequestException` naming the exact URL instead of going to the network. `tests/Feature/Support/StrayRequestGuardTest.php` is that guard's own witness — delete the call and it reds.

⚑ **What the escape cost, measured on card#7225 and re-measured suite-wide on card#7300.** Adding one outbound read to `board_create_card` left every board-tools test that reaches a success arm issuing a real call to the fixture host, whose connection failure was swallowed by the caller's own fail-soft `catch` — **all 74 tests in the three board-tools suites — `AgentToolsCallTest` + `BoardToolDispatcherTest` + `ToolsCallCommandTest`, counted at `35785c9`, the tip the change branched from; the fourth file in `tests/Feature/AgentTools` is `BoardToolAgentResolverSshTest`, and the directory stands at 83 cases today — passed while the new read was never once exercised as intended**. The fall-through itself is measured, not inferred: with the guard removed, an unmatched request under the array form raises `ConnectionException: cURL error 6: Could not resolve host: kanban.example.com` from the live handler. Read the number as *the request left the process*, not as a fixed code: it is whatever the live handler returns for the host in hand, and 6 is what a non-resolving fixture host yields. ⛔ **A resolving one is the dangerous case, and this repo had one:** the crosstalk leg of `tests/Feature/Config/InstallGuardTest.php` read `bridge.config_dir` off the `BRIDGE_DIR` a deployed checkout's `.env` sets — `phpunit.xml` overrides neither — so on an operator host it ran `bridge:check` against the REAL install and issued four token-bearing requests to that install's live board and to `api.github.com` on every suite run, while on CI (a fixture `BRIDGE_DIR` that does not exist) the same test exercised nothing of the sort. Same test, two subjects, decided by the host.

⛔ **The guard is the floor of the rule below, NOT a replacement for it.** `StrayRequestException` extends `RuntimeException` and `PendingRequest::makePromise()` re-throws it ahead of the `ConnectionException` wrapping — so it reaches the caller, and a caller that degrades softly on `catch (Throwable)` (the correct production posture, e.g. `KanbanMoveCardHandler`'s no-regression guard) swallows it exactly as it swallowed the connection error. **The test then stays GREEN on the degradation arm, and the guard is silent about it.** Measured on card#7300 by instrumenting the throw site and running the whole suite: of the strays the guard surfaced, only two reached a red test; every other one was swallowed and its test passed. Two things follow, and neither is optional:

- **When a change adds an API call, extend the stub set of every test that reaches it AND assert the new request on the wire (`Http::assertSent`).** The wire assertion is what makes the stub load-bearing where the caller swallows: drop the stub and the request is never recorded, so the assertion reds. Nothing else in the suite will tell you.
- **Do not read a green suite as evidence a call was faked** — and be especially wary where the caller degrades softly. A test registering a catch-all (`Http::fake()` with no arguments, or a `'*'` pattern) has answered every request it will ever make, so the guard can say nothing at all about it.

The population is derived, never grepped: `grep`ping for `Http::fake` cannot bound it, because a class that calls `Http::fake()` and stubs some of its URLs escapes on the others. Turn the guard on, run the suite, and read the strays off the run.
