...
The server is two files: the entry point `agent-webhook-bridge-channel.mjs` and its sibling `channel-lib.mjs` (a handful of pure helpers, imported relatively). Copy or symlink the **whole directory** (as above) so both travel together — cherry-picking only the entry file breaks the import, and nothing will tell you until the MCP server fails to launch at the next session start.
