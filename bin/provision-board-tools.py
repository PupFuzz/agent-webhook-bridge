...
`_require_win_openssh`, and the icacls ACL enumeration/decision) are spec-complete
against aimla's AC-1..6 (roundtable #141) but are UNPROVEN until a Windows/separate
seat runs them — this box is Linux and cannot execute the Windows paths in the test
suite. The `--self-cert` ssh -i round-trip (spec §5.6) is the mechanism that converts
this leg from spec-complete to proven the moment a Windows seat runs it clean; the
icacls SID-based ACL assertion is defense-in-depth, not the authoritative perm check.
Do NOT treat the Windows leg as certified on reasoning alone.
"""
...
def _make_name_to_sid(owner_name, owner_sid):
    ...
    (fail closed). Name matching is the locale-dependent seam; the well-known accounts
    resolve to fixed SIDs so the DECISION is locale-independent for the tolerated set.
    """
    known = {
...
def _require_win_openssh() -> None:
    ...
    the Node precheck) — the whole leg is `-i <key>` over ssh.exe/ssh-keygen.exe."""
    for exe in ("ssh.exe", "ssh-keygen.exe"):
