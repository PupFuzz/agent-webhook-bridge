...
| `GET /tasks/{id}.json` exposes `last_stage_move.actor_type`, and `"service"` identifies a machine-set stage move | kanban | The coord-card revive gate (DL-200) is an **allow-list** on `actor_type === "service"` and **fails CLOSED**: absent, null (a card predating the field), or any unrecognized value ⇒ **no revive**. So a kanban that drops/renames the field doesn't mis-revive — it silently stops reviving, and a human-set terminal is never reversed. Degrading toward "never revive" is the safe direction; the consumer's reconcile backstops. | bridge DL-200 |
