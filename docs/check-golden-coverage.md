...
> 77 branch predicates in `handle()`, in 59 minutes. Decision record: **DL-242**.
...
|---|---|---|
| `elseif-L353` | elseif | `SecretFile::isInsecure($secretPath)` |
| `elseif-L392` | elseif | `! is_writable($dir)` |
| `if-L422` | if | `$conn !== false` |
| `if-L458` | if | `is_file($httpMarker)` |
| `if-L467` | if | `$conn !== false` |
| `if-L493` | if | `$configs !== [] && is_string($configDir)` |
| `foreach-L495` | foreach | `$registry->collisions()` |
| `foreach-L498` | foreach | `$configs` |
| `if-L556` | if | `! $client->byRefAvailable($firstBoard)` |
| `if-L584` | if | `config('bridge.writeback.correlation', 'ref') !== 'ref' && $vis['total'] > KanbanClient::SEARCH_LIMIT * Kanba…` |
| `if-L592` | if | `! in_array($mapping->swimlaneId, $client->boardSwimlaneIds($mapping->boardId), true)` |
| `if-L608` | if | `$missing !== []` |
| `if-L633` | if | `! in_array('issue_number', $present, true)` |
| `foreach-L653` | foreach | `$mapping->startedFromStages ?? []` |
| `foreach-L659` | foreach | `$mapping->unparkFromStages ?? []` |
| `if-L664` | if | `$mapping->coordCardStageId !== null` |
| `if-L669` | if | `$mapping->coordCardTerminalStageId !== null` |
...
| predicate | kind | condition |
|---|---|---|
| `elseif-L103` | elseif | `! is_dir($configDir)` |
| `if-L112` | if | `! is_string($secretDir) \|\| ! str_starts_with($secretDir, '/')` |
