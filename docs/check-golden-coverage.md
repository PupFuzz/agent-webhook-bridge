...
> 102 branch predicates in `handle()`, in 66 minutes. Decision record: **DL-242**.
...
|---|---|---|
| `elseif-L393` | elseif | `SecretFile::isInsecure($secretPath)` |
| `elseif-L432` | elseif | `! is_writable($dir)` |
| `if-L462` | if | `$conn !== false` |
| `if-L498` | if | `is_file($httpMarker)` |
| `if-L507` | if | `$conn !== false` |
| `if-L537` | if | `$configs !== [] && is_string($configDir)` |
| `foreach-L539` | foreach | `$registry->collisions()` |
| `foreach-L542` | foreach | `$configs` |
| `if-L691` | if | `config('bridge.writeback.correlation', 'ref') !== 'ref'` |
| `if-L731` | if | `$mapping->stageFor('merged') !== null && $mapping->stageFor('merged') === $mapping->stageFor('merged_to_main')` |
| `if-L739` | if | `! $fromFile` |
| `if-L757` | if | `! $client->byRefAvailable($firstBoard)` |
| `if-L785` | if | `config('bridge.writeback.correlation', 'ref') !== 'ref' && $vis['total'] > KanbanClient::SEARCH_LIMIT * Kanba…` |
| `if-L793` | if | `! in_array($mapping->swimlaneId, $client->boardSwimlaneIds($mapping->boardId), true)` |
| `if-L809` | if | `$missing !== []` |
| `if-L834` | if | `! in_array('issue_number', $present, true)` |
| `foreach-L854` | foreach | `$mapping->startedFromStages ?? []` |
| `foreach-L860` | foreach | `$mapping->unparkFromStages ?? []` |
| `if-L865` | if | `$mapping->coordCardStageId !== null` |
| `if-L870` | if | `$mapping->coordCardTerminalStageId !== null` |
...
| predicate | kind | condition |
|---|---|---|
| `elseif-L99` | elseif | `! is_dir($configDir)` |
| `if-L108` | if | `! is_string($secretDir) \|\| ! str_starts_with($secretDir, '/')` |
