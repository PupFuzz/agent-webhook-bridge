...
> 38 branch predicates in `handle()`, in 27 minutes. Decision record: **DL-242**.
...
|---|---|---|
| `if-L329` | if | `$configs !== [] && is_string($configDir)` |
| `foreach-L331` | foreach | `$registry->collisions()` |
| `foreach-L334` | foreach | `$configs` |
...
| predicate | kind | condition |
|---|---|---|
| `elseif-L123` | elseif | `! is_dir($configDir)` |
| `if-L132` | if | `! is_string($secretDir) \|\| ! str_starts_with($secretDir, '/')` |
