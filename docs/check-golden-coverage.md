...
> 48 branch predicates in `handle()`, in 36 minutes. Decision record: **DL-242**.
...
|---|---|---|
| `foreach-L209` | foreach | `$cfg->subscriptions` |
| `if-L210` | if | `$sub->provider === 'github'` |
| `if-L341` | if | `$configs !== [] && $ctx->configDir !== null` |
