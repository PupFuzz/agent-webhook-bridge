...
> 56 branch predicates in `handle()`, in 41 minutes. Decision record: **DL-242**.
...
|---|---|---|
| `elseif-L305` | elseif | `SecretFile::isInsecure($secretPath)` |
| `elseif-L344` | elseif | `! is_writable($dir)` |
| `if-L374` | if | `$conn !== false` |
| `if-L410` | if | `is_file($httpMarker)` |
| `if-L419` | if | `$conn !== false` |
| `if-L445` | if | `$configs !== [] && is_string($configDir)` |
| `foreach-L447` | foreach | `$registry->collisions()` |
| `foreach-L450` | foreach | `$configs` |
...
| predicate | kind | condition |
|---|---|---|
| `elseif-L113` | elseif | `! is_dir($configDir)` |
| `if-L122` | if | `! is_string($secretDir) \|\| ! str_starts_with($secretDir, '/')` |
