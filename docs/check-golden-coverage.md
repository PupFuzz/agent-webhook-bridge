...
> 58 branch predicates in `handle()`, in 43 minutes. Decision record: **DL-242**.
...
|---|---|---|
| `elseif-L357` | elseif | `SecretFile::isInsecure($secretPath)` |
| `elseif-L396` | elseif | `! is_writable($dir)` |
| `if-L426` | if | `$conn !== false` |
| `if-L462` | if | `is_file($httpMarker)` |
| `if-L471` | if | `$conn !== false` |
| `if-L497` | if | `$configs !== [] && is_string($configDir)` |
| `foreach-L499` | foreach | `$registry->collisions()` |
| `foreach-L502` | foreach | `$configs` |
...
| predicate | kind | condition |
|---|---|---|
| `elseif-L107` | elseif | `! is_dir($configDir)` |
| `if-L116` | if | `! is_string($secretDir) \|\| ! str_starts_with($secretDir, '/')` |
