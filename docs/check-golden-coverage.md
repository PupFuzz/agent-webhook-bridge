...
> 35 branch predicates in `handle()`, in 25 minutes. Decision record: **DL-242**.
...
|---|---|---|
| `if-L346` | if | `$configs !== [] && $ctx->configDir !== null` |
...
| predicate | kind | condition |
|---|---|---|
| `elseif-L133` | elseif | `! is_dir($configDir)` |
| `if-L142` | if | `! is_string($secretDir) \|\| ! str_starts_with($secretDir, '/')` |
