...
> 58 branch predicates in `handle()`, in 45 minutes. Decision record: **DL-242**.
...
|---|---|---|
| `elseif-L347` | elseif | `SecretFile::isInsecure($secretPath)` |
| `elseif-L386` | elseif | `! is_writable($dir)` |
| `if-L416` | if | `$conn !== false` |
| `if-L452` | if | `is_file($httpMarker)` |
| `if-L461` | if | `$conn !== false` |
| `if-L487` | if | `$configs !== [] && is_string($configDir)` |
| `foreach-L489` | foreach | `$registry->collisions()` |
| `foreach-L492` | foreach | `$configs` |
...
| predicate | kind | condition |
|---|---|---|
| `elseif-L108` | elseif | `! is_dir($configDir)` |
| `if-L117` | if | `! is_string($secretDir) \|\| ! str_starts_with($secretDir, '/')` |
