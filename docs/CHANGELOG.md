...
- **card#8425 (DL-325)** — **a periodic-job REGISTRY inside the bridge: jobs are DATA, handlers are CODE, and ONE opt-in tick joins the after-response event gate rather than replacing it.** ⚠ **RUN `php artisan migrate`** — one new table, `scheduled_jobs`. **⚠ THIS ADDS A NEW INGRESS AND NEW PERIODIC EXECUTION**, and **an install that changes nothing behaves exactly as it did**: the registry ships EMPTY, it runs from the inbound webhook's after-response gate (DL-199's shape — bounded, non-blocking, never on a client-visible path), and **adopting the tick is OPT-IN**.
  ...
  **What an operator must do to adopt it** — two things, both optional: (1) add ONE crontab line under the seat-owner account, **never root** — `0,10,20,30,40,50 * * * * cd /path/to/bridge && php artisan bridge:tick`; (2) declare its interval, `BRIDGE_JOBS_TICK_EXPECTED_EVERY=600`, so a dead line goes LOUD instead of sitting green (a `.env` edit is inert under `config:cache`). ⛔ **You never add a second crontab line** — everything else is a row in the registry, insertable and removable at runtime by any code path.

  **What the tick buys, and why the event gate is not enough:** DL-306 recorded the gap against itself — *"the pass fires on the first inbound webhook AFTER the interval lapses, so an install receiving nothing pushes nothing"* — so **an install with no traffic runs no periodic work at all**, and no arrival-gated mechanism can fix that because the gate's clock IS the traffic. The two ingresses cover different blind spots: the gate covers the BUSY install, the tick covers the SILENT one. ⚠ **This AMENDS DL-199, and corrects its stated reason rather than merely narrowing it**: the fleet's no-standing-daemons posture was **anti-proliferation** — it existed to stop multiple agents each reaching for cron as the easy way out — not an absolute prohibition, and one crontab line with the bridge owning the job list satisfies it. DL-199's own reasoning and all four of its implementation properties stand and are inherited verbatim.

  **Governance, encoded structurally.** A job entry may only reference a handler that exists in bridge code, so what a job *can do* is fixed at code-review time; an unknown handler is a **LOUD refusal at insert AND at run** (the instance records `refused` with a reason, the scheduler logs at error, and `bridge:check` **fails**) — never a silent skip. A handler declaring the state-mutating capability is **INERT** until this install names it in `BRIDGE_JOBS_ARMED_MUTATORS`; read-and-alert handlers need no entry. **Instances are free** — insert/remove is ungated and runtime.

  ⭐ **A periodic job is the LAST resort, and the docs and the API both say so before they say anything else.** `docs/periodic-jobs.md` opens with a four-step decision order that points at the event gate, a consumer pull, and do-it-when-it-happens **before** it will describe a job; and **every instance carries a REQUIRED `justification`** — one sentence on why the work cannot be event-driven, refused at insert if absent, returned by the enumeration API and printed under every row by `bridge:jobs`, so the registry audit answers *"why is this periodic?"* and not only *"what runs?"*. It is **not** an approval gate: nobody is consulted, insertion stays programmatic and runtime, and the answer is never judged. ⚠ Its bound is stated rather than implied — the floor refuses a blank, a dash and an `n/a`; it cannot refuse a fluent sentence that is wrong.
  ...
  ⚑ **The tick's EXIT CODE is part of its contract, because a crontab line can read nothing else.** `bridge:tick` (and `bridge:jobs run`) exit **0** whenever the pass ran — jobs that FAILED or were REFUSED inside it are recorded on their own rows, not in the exit code — and **0 on every ordinary skip** (another pass holds the lock, the shared minimum interval has not elapsed, the registry is switched off). They exit **non-zero only when the pass could not run at all**: an unreachable database, a dead cache backend, or a `BRIDGE_JOBS_*` cadence this install cannot act on. A line reddening on ordinary skips would mail its operator on most runs of a busy install. ⛔ **A cadence value outside its bound is REFUSED, never clamped** — `BRIDGE_JOBS_MIN_PASS_INTERVAL=sixty` reads as `0`, and flooring that to 1 would turn an intended 60-second floor into a pass per second, silently; instead no pass runs, `bridge:check`'s `jobs.posture` leg **fails** naming the key, and the tick exits non-zero. ⚑ The last-pass-failure marker is cleared only by a pass that actually RAN, so an ordinary skip seconds later cannot erase it. ⛔ **A DEAD CACHE BACKEND RIDES THAT CONTRACT — and all three of the app's after-response shells used to break on it.** The catch arm that promises *"nothing escapes"* recorded its fault by WRITING TO THE CACHE, unguarded, as its first statement; with the cache as the fault the arm re-raised, so `bridge:tick` died at the tick stamp with a stack trace (exit 1 *by accident* — no summary line, no log line, no marker) and the after-response ingress ended every delivery with an unhandled fatal in the FPM worker. Fault recording now goes through one primitive, `App\Bridge\Support\FaultMarker`, which **logs first and writes the marker second, each leg guarded** — and the byte-identical arms in `RetentionGate` (DL-199) and `StandupGate` (DL-306) are routed through it in the same change, because all three carried the defect and every test of the promise broke the DATABASE, which leaves the recorder working. ⚠ On a dead cache store the LOG is the only surviving record: the marker lives in the store that failed. A tick whose stamp cannot be written reads as `unmeasured`, never as fresh.

  **New:** `php artisan bridge:jobs [list|add|remove|enable|disable|run] [--json] [--assert-tick]` (the audit surface — it filters nothing, leads with the justifications, and prints **every** stored column on both the human listing and `--json`, which is what makes the whole-population claim and the no-secrets rule honest — a claim held by a test that reads the column list off the SCHEMA, so a column added to `scheduled_jobs` and printed on neither surface reds rather than quietly widening the space the rule is stated over), `php artisan bridge:tick`, `App\Bridge\Scheduling\*`, `App\Models\ScheduledJob`, `docs/periodic-jobs.md`, five `BRIDGE_JOBS_*` keys (all with safe defaults, all validated in one place — `App\Bridge\Scheduling\JobsConfig`, the `RetentionConfig` shape), and ONE shipped handler — `standup_digest`, which drives the existing DL-306 digest from the tick through the SAME pass the event gate uses, so both ingresses still push at most one digest per `BRIDGE_STANDUP_INTERVAL`. **No receiver accept/reject change, no token-scope change, no change to any existing handler's behaviour.**
