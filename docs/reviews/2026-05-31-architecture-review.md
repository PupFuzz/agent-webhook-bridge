...
- **B-13. Regex anchor latent bug (Security S4).** `ProviderName`/`ScopeId` patterns lack the `D` modifier (or `\z`), so `$` matches before a trailing `\n` — a trailing-newline injection could slip a second line past the anchor. Low exploitability (provider from route, scope re-checked vs body) but add `D`/`\z` + a test.
- **B-14. Constrain classifier-supplied `socket` paths (Security S2).** The `url` transport is loopback-gated but a classifier-supplied `socket` has no path-prefix constraint — a custom classifier could point `channel_push` at another tenant's UDS. Constrain to a configured state-dir prefix (same custom-classifier surface class as B-3).
...
- **B-17.** `bridge:check` assertion that the config dir is actually 0700 / not group-writable on a multi-tenant host (Sec N3).
