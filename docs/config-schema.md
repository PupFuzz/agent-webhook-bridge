...
| `wake_membership` | list<string> | `[to_me, to_all]` | **`coord-message` family (DL-190).** Which label classes grant coord-message **live-wake** (channel_push): `to_me` (a `to:<me>` label), `to_all` (`to:all`), `from_me` (`from:<me>` — activity on a thread you opened). **DEFAULT is narrow `[to_me, to_all]`** — over-wake is the guarded failure mode; a coordinator opening many threads would else wake on every reply to them. Add `from_me` to also live-wake on **all** activity on threads you opened (a `from:<me>` issue label). Note: a comment's direction lives in its **body `TO:` line**, not as a `to:<me>` issue label, and the body-`TO:` check only *narrows* within membership (never grants) — so dropping `from_me` shifts **even a directed reply on your own thread** from live-wake to SessionStart (`from:<self>` / `protocol:invalid`) boundary pickup, not just the no-`TO:` case. That reconcile is the deliberate backstop. The comment-`TO:` narrowing itself is unaffected; this gates membership only, and inbox-staging is unchanged. Values lowercased. |
...
| `impl_wake_emit` | string | `channel_push` | **`impl-ci-wake` family (DL-190).** How a **wake-worthy** impl event (release-landed / failed-CI / provenance-success) wakes. `channel_push` (default) = the classifier hand-emits a surgical `channel_push` (the pre-DL-190 behavior). `none` = stage the `Intent` but emit **no** `channel_push`, so a channel whose `route_intents:true` owns waking isn't double-woken. Any other value falls back to `channel_push`. |
