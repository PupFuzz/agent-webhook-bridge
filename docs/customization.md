...
- `app/Bridge/Classifiers/CoordinationClassifier.php` — the config-driven, config-gated superset (roundtable #8): `extends InboxOnlyClassifier` (so it inbox-stages kanban `task.*` events) and runs the `classifier.config.families` you enable — `coord-message` (GitHub coordination messages), `impl-ci-wake` (push→release-branch + CI wake), `kanban-triage` (below). Default `[coord-message]` = the pre-#8 behavior. See [`config-schema.md`](config-schema.md) § `classifier.config`.
