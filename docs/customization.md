...
| **Handler** | Implements `Handler` contract; dispatched by name synchronously in the same request. | `afterResolving(HandlerRegistry::class, fn ($r) => $r->register(name, instance))` in a `ServiceProvider`. | The always-shipped set is `HandlerRegistry::known()` (the constructor's table in `app/Bridge/Support/HandlerRegistry.php`, whose docblock names the DL behind each writeback handler); the writeback handlers are all inert without `writeback.json`. `spawn_detached` is opt-in (`BRIDGE_SPAWN_ENABLED`, DL-011). |
| **Periodic job handler** (DL-325) | Implements `App\Bridge\Scheduling\JobHandler`; run on a cadence by the job registry, from the after-response event gate and/or the opt-in `bridge:tick`. | `afterResolving(JobHandlerRegistry::class, fn ($r) => $r->register(new MyJob))` in a `ServiceProvider` **both the FPM worker and the CLI load** — a handler wired anywhere else exists on one ingress and is a loud refusal on the other. | The shipped handlers are listed in [`periodic-jobs.md`](periodic-jobs.md) § *Shipped handlers*. ⛔ **Read `docs/periodic-jobs.md` first — a periodic job is the LAST resort here**, and an instance is refused unless it states why the work cannot be event-driven. A handler declaring `JobCapability::MutatesState` is inert until the install arms it. |
...
- `app/Bridge/Classifiers/CoordinationClassifier.php` — the config-driven, config-gated superset (roundtable #8): `extends InboxOnlyClassifier` (so it inbox-stages kanban `task.*` events) and runs the `classifier.config.families` you enable — `coord-message` (GitHub coordination messages), `impl-ci-wake` (push→release-branch + CI wake), `kanban-triage` (below), `coord-card-create` (real-time coord issue → card, DL-198; and, on an `issues.edited` that changed the title, the card-name restamp, DL-341), `coord-card-move` (coord issue close→terminal / reopen→revive, DL-200), `coord-card-relane` (a coord `[TASK]` labelled `stage:*` after it was carded → lane move, card#6393). Default `[coord-message]` = the pre-#8 behavior; the three coord-card families are opt-in and additionally gated on the repo's `writeback.json` mapping (`create_coord_cards` / `move_coord_cards` — `coord-card-relane` rides `move_coord_cards` and additionally needs `coord_card_lane_stage_ids`). See [`config-schema.md`](config-schema.md) § `classifier.config`.
- `app/Bridge/Classifiers/KanbanTriageClassifier.php` — **back-compat shim** for the triage-wake (DL-168): a thin subclass of `CoordinationClassifier` that defaults its family set to `[kanban-triage]`. Set `classifier.class: App\Bridge\Classifiers\KanbanTriageClassifier` on the **triage-owner agent** and subscribe it to `task.created`, and a **human-filed, untriaged** card (no `triaged`/`id:pr:*` tag, no `dl` ref — read from the kanban DL-164 `card` snapshot, no API call) wakes that agent's session via `channel_push`. Registered-agent-filed and already-classified cards don't wake; the bridge's own creations are suppressed while a global echo id is configured (see `CoordinationClassifier`'s no-self-wake docblock, which owns the mechanism-by-path detail). Requires kanban **v0.22.0+**; other agents keep `InboxOnly` and never wake. Equivalent to `classifier.class: …\CoordinationClassifier` with `config.families: [kanban-triage]` — prefer the latter directly for new installs.
- `app/Bridge/Handlers/` — shipped handlers. Which are registered is `HandlerRegistry::known()`, read from the constructor's table in `app/Bridge/Support/HandlerRegistry.php` rather than listed here; `spawn_detached` joins it only when `bridge.spawn.enabled` is set
...
- **Classifier throws → event recorded + acked 200 (treatment A).** A deterministic bug must not wedge delivery into an 11-day retry storm. The raw event is stored; fix the classifier and `bridge:replay` — **while the event still has its payload** (`retention.null_payloads_older_than`, default 7d: replay REFUSES a payload-nulled event, DL-315; see [`CLAUDE_DEPLOYMENT.md`](../CLAUDE_DEPLOYMENT.md) § *Retention config (DL-199)*). The classify pass does not get a second chance on the same delivery.
...
- **Failures are recorded, not retried automatically.** A handler throw writes `done-with-note` to `agent_dispatches`. The intent is already durable in `inbox.jsonl`. To retry: `php artisan bridge:replay <N>` — **while the event still has its payload** (`retention.null_payloads_older_than`, default 7d: replay REFUSES a payload-nulled event, DL-315; see [`CLAUDE_DEPLOYMENT.md`](../CLAUDE_DEPLOYMENT.md) § *Retention config (DL-199)*). Treatment C: one agent's handler failing does not fail the delivery or affect other agents.
