...
2. A github-subscribed agent runs `GitHubPrCardMoveClassifier`, which:
   ...
   - **(opt-in with `create_coord_cards`) an `issues.edited` that CHANGED THE TITLE of a carded coordination issue** → emits a MOVE-LESS `kanban_coord_card` reaction (`disposition: renamed`) carrying both the new title and the PREVIOUS one (`changes.title.from`), so the handler can restamp a coord card name the bridge still owns — see *Optional: real-time coordination issue → card* below (DL-341). This arm is on the `CoordinationClassifier`'s `coord-card-create` family, not on the PR classifier; every other `issues.edited` (a body change) stays the no-op it was.
...
| `task.update` | every non-stage-only field PATCH — the draft `block_reason` overlay, the `payload` correlation stamp, the **DL-328 dependabot name restamp** and its **DL-341 coord-card twin** (`{name}` alone, on a card whose name is still the one the bridge stamped), and (DL-326) a `board_correct_card` correction of `name`/`description`/`tags` |
...
| move / closed-unmerged | yes, and **PINNED** | **refused** — the lifecycle move and the closed-unmerged archive are both withheld (DL-335, see below); the DL-328 **restamp is not** — see below. ⚠ NOT the create-race collapse: a pinned DUPLICATE twin is still retired |
...
**A PINNED card is refused on the two writes DL-335 covers (card#8454).** A non-empty `block_reason` **or** a `no-automove` tag is the same DL-178 human hold the move handler honours on every outcome, and until this shipped the dependabot handler was the one event-path mover that never read it — so a closed-unmerged dependabot PR **retired a card a human had parked**, while `bridge:reconcile` and the release-promote sweep both skipped it and the backstop could not repair what the event path had taken off the board (card#8289's asymmetry, one handler over). The refusal is loud — a `Log::warning` plus an `alert_channel` signal under `pinned_no_automove`, the same reason the move handler emits. ⛔ **There is no override here:** the DL-194 unpark and the DL-195 revive are `started` / `reopened` outcomes a dependabot PR never produces. ⚠ **The card stays correlated** — nothing is unstamped and nothing is created, so lifting the pin lets the PR's next event complete the move; a close, however, is delivered by GitHub once, so a card pinned through its own close stays live on the board until someone archives it by hand. That is the intended reading of a hold. ⛔ **One write is NOT covered, and it is stated here rather than left to the word "refused":** where a repo+PR correlates to **more than one** card (a create race), `CardCollapse::toSurvivor()` runs *before* the move consult and is pin-blind, so a pinned **duplicate twin** is archived anyway — only the survivor's move is withheld. The collapse retires a bridge-minted twin rather than acting on a card's lifecycle and is shared with a caller outside this regime, so widening the pin into it is its own ruling (DL-335 alternative (b)); the uncomfortable half is that the twin a human notices is the twin a human pins. ⚠ **One alert per repo, not per card:** this handler's dedup tuple carries the synthetic `dependabot_card` outcome (see *the alert reasons* below), so the first pinned card in a repo signals and later ones reach the durable log only. ⚠ **The DL-328 name restamp is NOT one of them, and this states it rather than leaving it to the word "refused":** DL-335 was ruled before that arm existed and never widened onto it, so a pinned card whose name is still the one the bridge stamped is restamped anyway. That is a narrower write than the two above — it touches `name` alone, moves no card and retires none — but whether a hold should stop it is a ruling nobody has made, and it is filed as such rather than decided here.
...
- **Create-only + idempotent.** This create path never moves or archives a card (nor does the DL-341 RETITLE arm below — it writes a `name` and nothing else). (The bridge as a whole is no longer create-only for coord cards: its sibling **`move_coord_cards`** (DL-200, a guarded fleet default since DL-204 — below) carries close→terminal / reopen→revive, and the opt-in `coord-card-relane` family (card#6393) carries lane→lane on a later `stage:*` label. The reconcile still owns column/lifecycle wherever the move leg is **off**, and **archival remains the reconcile's alone**.) It correlates by the **`id:<sid>` tag**: if a card already carries it, it **skips** — which covers redelivery, opened+reopened, **and** the bridge-vs-reconcile race (both movers key on the same tag). After a create it re-reads by tag and collapses a raced duplicate (keep lowest id, archive the rest — the shared deterministic tie-break). Durable, transient(5xx→retry)/permanent(4xx→log+no-op).
- **An upstream RETITLE restamps the card name — but only a name the bridge still owns (DL-341).** A coordination issue's title is edited in place (a `[QUERY]` sharpened, a `[TASK]` re-scoped), and a card named once at birth then asserts a title the thread has moved on from — the same defect DL-328 fixed for dependabot cards, on the issue-driven family. On an `issues.edited` carrying a title change the bridge writes the new title onto the correlated card — **gated on the card's current name being byte-identical to the issue's PREVIOUS title**, which GitHub delivers on that same event as `changes.title.from` and which is exactly the string the bridge stamped when it minted the card. A card whose name differs by so much as a space was renamed by somebody, and is left alone: trading a stale machine name for a destroyed human one is not an improvement, and the classifier hands the handler the issue title on *every* event, so an unconditional write would clobber that rename on the next webhook. Nothing here reads the *shape* of a name. **The correlation key comes from `changes.title.from`, not from the new title** — `id:<sid>` is derived from the anchored `[PREFIX]`, so a prefix-changing edit moves the sid and keying on the new title would search for a tag no card carries. Where >1 card correlates (a create race) **every** card whose name still matches is restamped and **none** is collapsed — retiring a raced twin belongs to the create path.

  > ⚑ **It writes `name` and MOVES NOTHING, and that is a ruling.** A retitle across the anchored `[TASK]` prefix changes whether the lane model would govern the thread at all (`CoordLaneStages::governs()` reads that title), and the restamp still does not re-derive a stage. Three reasons agree: a coord-card **move** is a separate opt-in (`move_coord_cards`, DL-200) and this arm runs under `create_coord_cards`; a title edit **declares no lane**, so a derivation would demote an unlabelled card to the `later` default on a rename — the invented sequencing decision `coord-card-relane` excludes the `unlabeled` action to avoid; and in the un-governing direction the only target is the fixed `coord_card_stage_id`, which is a rewrite of wherever the card sits. **A `stage:*` label edit is what states a lane, and `coord-card-relane` already carries it.**

  > ⚠ **Bound: the evidence lives on the rename event.** A retitle the bridge never received — it was down, or the repo opted in afterwards — leaves a card no later event can prove ownership of. The periodic reconcile is the backstop for those; this leg does not try to repair them. The write is a flat `PATCH {"name": …}` and touches no other field, so a lane, column, tag or payload edit made in the meantime survives it.

  > ⛔ **Two stated gaps, both deliberate.** (1) **The by-ref population is not covered:** under `issue_population: all` a non-prefixed issue's card carries no `id:` tag and is correlated by-ref, and the restamp resolves by tag alone — those cards keep their birth name after a retitle. (2) **A prefix-changing retitle leaves the `id:<sid>` TAG stale:** the card keeps `id:QUERY-N` while its name says `[TASK]`. ⚠ **That name-vs-tag disagreement is NEW with this leg**, and is stated that way rather than as an inherited one: before the restamp, a prefix edit left the card's name and its tag stale together, and only a name write moves one without the other. It is left unrepaired because it is **inert** — nothing correlates on a card's name. The bridge resolves this card set by the tag alone (`cardsByTag` / `cardRowsByTag`), and the consumer's reconcile keys on the `id:` tag, force-preserves it, and writes a coord card's `name` only when it creates the card. Re-tagging is a correlation-key rewrite shared with that reconcile, not a name write.

  > ⚠ **A PINNED card is restamped.** Neither arm of this handler consults the DL-178 pin (`block_reason` / `no-automove`); DL-335 widened the pin onto the dependabot archive and move only, and the DL-328 restamp is not covered there either. Whether a hold should stop a name write is a ruling nobody has made — stated here rather than decided.
...
**Which failures signal.** **The tables below are the authority — there is no shorter rule that is true.** Since DL-285 the coverage is near-total: every permanent-refusal branch in all **six** writeback handlers signals — as does the one CLI refusal, `bridge:reconcile`'s board re-check (DL-301) — with the **two** accounted-for exceptions named in *Still log-only* below (plus the inherent `writeback_not_configured` degradation, which routes through the same primitive but has no channel to reach). A test — `tests/Feature/Writeback/WritebackRefusalSignalCoverageTest.php` — re-derives that population from the handler sources on every run and reds on a `Log::warning`/`Log::error` that is neither routed nor listed, so a new arm in those files cannot re-mint the omission. **Stated bound on that guard:** its population is the `app/Bridge/Handlers/Kanban*Handler.php` glob, so a future writeback handler named outside that pattern is not covered until the glob is widened. (Its *second*, level-independent leg does not share that population and is far wider: since **card#8530** it walks the WHOLE of `app/` on the tokenizer — the same derivation the pre-read enforcer uses — and dispositions each site it finds. It grew that way: handlers only, then `app/Console/Commands/Bridge/*.php` too from DL-301 because the reconcile arm was the first caller of the board guard outside the handler files, and then the whole tree, because a hand-written glob plus a stated bound for everything outside it is a declaration with no check.) The `Log::info` "not tracked" branches stay **quiet** — they're the normal case for an event the operator simply hasn't mapped, not a failure. **Second stated bound, and the sharper one:** that guard's population is the `Log::warning`/`Log::error` call sites, so a refusal *written at `Log::info`* is invisible to it. That is how `kanban_coord_card_move`'s belongs-to-mapped-board refusal stayed silent through DL-285's sweep and after it, until **card#7133**, while this section read as total coverage — it was found by reading the three copies of that guard side by side, not by the guard. A green run therefore says "no bare warning/error is unaccounted for", not "no refusal is silent".

**That bound still holds — and one refusal KIND is now closed underneath it (card#7138, DL-292).** The fix for the blind spot above is not to widen the population by level: `Log::info` is also every "not tracked", success and policy line in these handlers, and sweeping them in would drown the signal it exists to carry. It is to make the refusal that was missed *unwritable* anywhere else. The belongs-to-mapped-board rule and its refusal report now live in **one primitive** (`MappedBoardGuard::refuses()`) that all three arms call, and the same test carries a **second, level-independent leg**: it re-derives, every run, every site in the whole of `app/` naming the card field `board_id` or the reason `card_not_on_mapped_board` (on PHP's own tokenizer, so prose is excluded by construction; an array-literal KEY is excluded too, being a field written rather than a card's board read), and requires that set to equal the list it **dispositions** — each entry naming the SUBJECT the site reads and, where that subject is a card, why the compare is not a tenant refusal. The sites that read a card row and compare it outside the primitive all sit on the board-tools door, whose axis is `BoardToolsConfig` and not `WritebackMapping`; routing those through the primitive would mint a writeback refusal alert for a call that has no repo and, on most of them, succeeded (card#8530, DL-299/DL-302/DL-326). **Which sites those are, and what each one reads, is the `DISPOSITIONED` list in that test — read it there rather than a count restated here.** A membership decision cannot be written without reading the card's board, so a fourth copy reds — at `Log::info`, at `Log::warning`, or with no log line at all. **It closes exactly that one kind**; every other refusal kind is still covered only by the level-keyed leg and its bound above.
...
| | malformed **rename** payload — no `name_from`, no real title change, or no `id:<sid>` correlation key, so no evidence the name is the bridge's (DL-341) | `coord_card_rename_payload_invalid` | ✅ |
