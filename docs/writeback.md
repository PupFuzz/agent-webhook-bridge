...
2. A github-subscribed agent runs `GitHubPrCardMoveClassifier`, which:
   ...
   - emits a `kanban_move_card` durable reaction per correlated card (or no-ops if the repo is unmapped / no correlating token / no matching card).
...
**Which failures signal.** **The tables below are the authority — there is no shorter rule that is true.** Since DL-285 the coverage is near-total: every permanent-refusal branch in all **six** writeback handlers signals, with the single accounted-for exception named in *Still log-only* below (plus the inherent `writeback_not_configured` degradation, which routes through the same primitive but has no channel to reach). A test — `tests/Feature/Writeback/WritebackRefusalSignalCoverageTest.php` — re-derives that population from the handler sources on every run and reds on a `Log::warning`/`Log::error` that is neither routed nor listed, so a new arm cannot re-mint the omission. The `Log::info` "not tracked" branches stay **quiet** — they're the normal case for an event the operator simply hasn't mapped, not a failure.
...
**Still log-only — one branch, and it is unreachable.** `kanban_promote_released`'s *"mapping is missing the Shipped and/or Released stage"* is the only permanent `Log::warning` in a writeback handler that emits no signal. It is **type-narrowing, not a refusal an operator can hit**: `WritebackConfig::load` rejects a `promote_on_release` mapping missing either stage and validates every stage value as numeric, so no config that reaches the handler can leave either null. An alert there could never be seen to fail, and a check that cannot fail is a decoration. `KanbanPromoteReleasedHandlerTest` pins the fail-closed load that makes it unreachable, so if that guard is ever relaxed the branch becomes a real refusal and the test reds.

Two further **`Log::error`** sites are outside this section's scope and are recorded here so they are not mistaken for coverage: `kanban_dependabot_card`'s *"archive returned 200 but the card is not archived"* and its twin in the shared `CardCollapse` primitive, which has no `(repo, outcome)` tuple to dedup on. `KanbanClient`'s three correlation diagnostics (0-card read, page-ceiling, no-card-collection body) are the same shape one layer down. All four are tracked on card#5968 rather than left implicit.
