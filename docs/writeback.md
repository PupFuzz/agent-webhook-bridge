...
> **Shared-board safety.** Correlation keys on the *bare* PR number (the `github_pr` by-ref index isn't repo-qualified), so when several repos map to **one board** (a `swimlane_id` lane per repo — see above) a same-numbered PR in another repo also correlates. The handler attributes each correlated card to its repo (via the `github.com/<owner>/<repo>/pull/` segment of the card's `pr_url`) and only ever moves/archives **this repo's** cards — a co-hosted repo's identically-numbered card is left untouched.
