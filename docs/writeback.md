...
>
> **Two things are deliberately still silent, and are card#5961 — do not read their silence as coverage.** (1) A **Unicode-digit** DL (`DL-<U+0663>239`) warns on **neither** surface: the runtime probe is ASCII by ratification (DL-231), and unlike the card token there is **no DL arm on the CI PR-title lint** to cover the class by name. (2) A subject whose **card token parses** beside a malformed DL — `feat/card-3410-slug-DL_272` — moves card 3410 and silently fails to stamp its `dl_number`, because the near-miss probe sits behind the whole-subject guard (DL-234(e)): a correlating token means the card *does* move, so the subject is not in the silent-**move**-failure class the probe exists to catch, and the warning's own "no move" clause would be false about it. The lost **stamp** is a different signal and needs its own.
