...
2. A github-subscribed agent runs `GitHubPrCardMoveClassifier`, which:
   ...
   - **on a MERGE outcome, additionally requires an explicit CLOSING FORM in the PR title** naming the card that moves (`Closes card#N` / `Fixes DL-NNN`) — see *Mention vs closure* below. A PR that merely CITES a card moves nothing on merge; every other outcome is keyed on correlation alone, exactly as before.
   - emits a `kanban_move_card` durable reaction per correlated card (or no-ops if the repo is unmapped / no correlating token / no matching card / no closing form on a merge).
...
## Mention vs closure — what makes a merge move a card (card#7348 / DL-305)
...
**The rule.** On the two MERGE outcomes — `merged` and `merged_to_main` — a card moves only when the **PR title** carries a closing form naming it:

| PR title | On merge |
| --- | --- |
| `feat: widget rework, Closes card#4811` | card 4811 → `stages.merged` |
| `fix: thing (Fixes DL-239)` | every card DL-239 tracks → `stages.merged` |
| `feat: widget rework, follows card#4811` | **nothing moves** — a mention, warned in the log |
| `chore: release v1.2 — includes DL-239` | **nothing moves** |

The accepted verbs are GitHub's own linking keywords — `close`/`closes`/`closed`, `fix`/`fixes`/`fixed`, `resolve`/`resolves`/`resolved`, case-insensitive — and the token spellings are unchanged (the *Card-token correlation* section below is still the one place those are written for operators). `App\Bridge\Support\ClosureGrammar` is the authority; `bridge:check` renders its accepted set at you per mapping, so this table can go stale but the check cannot.

**Four properties worth knowing before you adopt it:**

- **A bare mention is a NO-OP, never a demotion.** The writeback re-classifies in-window PRs on every pass (redeliveries, `bridge:replay`, and `bridge:reconcile` on a schedule), so a rule that returned an *earlier* stage for a mention would not gently correct the mistaken cards — it would **mass-demote every already-correct card on the first run**, because every historical PR is a bare mention under the new grammar. Withholding the move instead leaves every existing stage exactly where it is. **Nothing needs backfilling on upgrade.**
- **The failure direction is safe.** A forgotten closing form leaves a card UNDER-promoted, which you fix by moving it. The old behaviour failed toward an irreversible stage.
...
- **The closing form must name the card that MOVES, through the same resolution that selected it** — the closure grammar COMPOSES with the existing foreign-mention discrimination rather than replacing it. `Closes DL-239` authorizes the cards DL-239 resolved to. It does **not** authorize a co-present `card#N` that the DL-218 foreign-mention guard just ruled authoritative — that DL is another card's work, and letting it through would re-open the hijack from the other side.

**What is NOT gated, and why:**

- **`opened` / `reopened` / `closed_unmerged` / `started`.** None of them claims a card is done. `opened` is reversible and is what stamps the card's PR refs so `bridge:reconcile` can find the PR later; `closed_unmerged` is an abandon disposition (revivable, DL-195); a branch ref cannot carry a closing verb, so gating `started` would make every branch-create inert.
...
**`bridge:reconcile` applies the identical gate**, on the same field, because it re-derives the same proposition from the same evidence; without that, the backstop would re-apply on a later pass exactly the move the event path had just declined.

**Adopting it costs an install with the usual convention nothing** — one card per PR, cited only when the PR closes it — but that is a *convention*, and it fails silently the first time someone cites a card for context. Change your PR-title habit to `Closes card#N` and the writeback and the habit say the same thing.
