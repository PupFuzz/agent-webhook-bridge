...
⚠ **Both qualifiers are load-bearing, and a recipe without them returns the wrong set** (card#8523 R2 measured the earlier `grep -rn "Log::warning\|Log::error" app/Bridge/Writeback/`: **17 lines**, seven of them *inside* `WritebackAlertNotifier` — the guard itself, the exact opposite of "outside its population" — plus docblock mentions that emit nothing). `--exclude` drops the guard; the leading-whitespace anchor drops prose. The first three are tracked on card#5968; the ledger's is named here because this recipe returns it.
