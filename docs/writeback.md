...
- **An upstream RETITLE restamps the card name — but only a name the bridge still owns (DL-341).** A coordination issue's title is edited in place (a `[QUERY]` sharpened, a `[TASK]` re-scoped), and a card named once at birth then asserts a title the thread has moved on from — the same defect DL-328 fixed for dependabot cards, on the issue-driven family. On an `issues.edited` carrying a title change the bridge writes the new title onto the correlated card — **gated on the card's current name being byte-identical to the issue's PREVIOUS title**, which GitHub delivers on that same event as `changes.title.from` and which is exactly the string the bridge stamped when it minted the card. A card whose name differs by so much as a space was renamed by somebody, and is left alone: trading a stale machine name for a destroyed human one is not an improvement, and the classifier hands the handler the issue title on *every* event, so an unconditional write would clobber that rename on the next webhook. Nothing here reads the *shape* of a name. **The correlation key comes from `changes.title.from`, not from the new title** — `id:<sid>` is derived from the anchored `[PREFIX]`, so a prefix-changing edit moves the sid and keying on the new title would search for a tag no card carries. Where >1 card correlates (a create race) **every** card whose name still matches is restamped and **none** is collapsed — retiring a raced twin belongs to the create path.
  ...
  > ⚠ **A PINNED card is restamped, and the two arms are NOT in the same position.** The RETITLE arm does not consult the DL-178 pin (`block_reason` / `no-automove`) — the pin governs a card's STAGE and its LIFECYCLE (DL-178 as annotated 2026-08-30, widened by DL-335's archive and by DL-340), and a `{name}`-only write is neither, so a hold does not stop this restamp by the rule as written rather than by omission. The CREATE arm above **does** consult it since DL-340 (card#8523): its post-create collapse refuses to archive a pinned twin, because that consult lives in the shared `CardCollapse` kernel. Whether a hold should stop a NAME write is a ruling nobody has made — stated, not decided, and it is the same question DL-328's dependabot restamp raises, so it is stated ONCE in the dependabot pin paragraph above and owned by **card#8557**.
...
**Outside the guard's population**, and recorded here so they are not mistaken for coverage: `CardCollapse`'s twin of the archive-contract error above; `KanbanClient`'s correlation diagnostics (0-card read, page ceiling, no-card-collection body) — the same shape one layer down, at a shared client with no per-event tuple; and, since card#8523, `PinGuard`'s **`pin_row_unreadable`** detector, which fires when a row reaches the pin consult carrying neither `block_reason` nor `tags` (kanban emits both on every row, so their joint absence is a degraded read and the pin predicate is about to answer "not pinned" for a card nobody could read); and `BoardDivergenceLedger`'s **`Log::error`** for an audit row that could not be persisted (log-only by the DL-020 posture — the write it audits has already landed, so re-running it would be worse than losing the row). **No count is quoted here** — the set moves — and the recipe that enumerates it is:

```bash
grep -rn --include='*.php' -E '^\s+Log::(warning|error)\(' app/Bridge/Writeback/ --exclude=WritebackAlertNotifier.php
```

⚠ **Both qualifiers are load-bearing, and a recipe without them returns the wrong set** (card#8523 R2 measured the earlier `grep -rn "Log::warning\|Log::error" app/Bridge/Writeback/`: **17 lines**, seven of them *inside* `WritebackAlertNotifier` — the guard itself, the exact opposite of "outside its population" — plus docblock mentions that emit nothing). `--exclude` drops the guard; the leading-whitespace anchor drops prose. Consolidating these diagnostics is tracked on **card#8586**, the live successor to card#5968 (`released_to_main`), which this line used to name.
