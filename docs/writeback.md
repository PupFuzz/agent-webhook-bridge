...
Every writeback **write to a card whose id was RESOLVED from kanban** now records both boards, from the guard's own rendering (`MappedBoardGuard::boardContext()`, the same pair the refusal arm emits): **`card_board`** — the card's own `board_id`, verbatim as kanban returned it — and **`mapped_board`**, the operator's configured board for the repo. Equal is the happy path; a divergence is the record doing its job. It replaces the old single `board` key on `kanban_move_card: moved` and `kanban_block_reason: <action>`, and it is **new** on the three `kanban_coord_card_move` legs, both dependabot arms and the promote arm, which recorded no board at all.

Two bounds, both deliberate. (1) **`card_board` is the card's RAW value, not normalised through the guard's predicate** — the accepted set is an interval (below), so `'8'` and `8.9` both belong to a mapping of 8 and which spelling kanban returned is exactly what a reader wants; and on the Group-B arms no membership compare runs at all, so normalising there would render an answer the code never asked. (2) **A CREATE carries `board` only** — `kanban_coord_card: created`, `kanban_dependabot_card: created` — because there is no resolved card to read a board from: the board is the POST target and the create response returns an id. That a created card's ACTUAL placement is never read back is a sibling shape, filed rather than fixed here. `tests/Feature/Writeback/WritebackSuccessBoardRecordTest.php` re-derives the write population every run and reds on a kanban write in the handler glob that is not accounted for, and on any handler that renders the pair by hand instead of calling the primitive.
