...
| Key | Type | Meaning |
| --- | --- | --- |
| `severity` | `"fail"` \| `"warn"` \| `"unvalidated"` \| `"ok"` | **`fail` is the only one that flips `ok` and the exit code.** `unvalidated` means *"I should have measured this and the install stopped me"* — a green run is not evidence about it. |
...
**⚠ The severity vocabulary is not yet precise, and that is disclosed rather than implied.** A check that reports `warn` *because it could not run* still reads as a finding here, not as a not-run. Re-assigning those sites is a separate, gated change (card#5291 / card#5292, plan stage 10). Until it lands, **do not infer "was measured" from the absence of `unvalidated`.**
