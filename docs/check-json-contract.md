...
⛔ **AND THEY ARE NOT SANITISED — a `message` can carry bytes a foreign principal wrote, verbatim, control characters included** (card#9121, DL-366). **Treat EVERY `message` as untrusted text, not as prose this install vouches for.** ⚠ **There is deliberately NO list of which fields do it, and the absence is the correction.** A previous revision of this section enumerated five categories of producer, and the very change that added the list invalidated it in the same commit by adding six more arms — an exhaustive prose list is a bare count in longer form, and a consumer who trusts it audits their join against a census that reads complete. The test is the PRINCIPAL, not the shape: a span is foreign when its bytes were chosen by anyone other than this install or its operator — another OS account, a remote host, a kanban card's author. ⚑ **Re-derive rather than read a list:** every span a producer has DECLARED is `App\Bridge\Support\Untrusted::span(` in `app/`, and a producer that interpolates a foreign string without declaring it is a producer anyway — which is exactly why no list here can be a bound. The **terminal** report escapes and length-caps the declared spans; **this document deliberately does not**, because these bytes are a write contract and changing them would move every consumer's payload at once with no version to key on. ⚠ **A consumer that renders a `message` to a terminal, a log viewer, a web page or a shell owns that encoding at its own boundary.**
...
| `message` | string | Operator prose. **Not part of the contract** (§2), and **not sanitised** — it may carry untrusted bytes verbatim (§2). |
