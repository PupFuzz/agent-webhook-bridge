...
| `bridge_side_unverified` | A block is present and **this run could not measure** this bridge's half: (ssh) the pinned-line probe returned `unvalidated` — the `authorized_keys` it needed was not readable by this run, typically because it was not root; (http) the bearer token file could not be seen or read by **this process** (cards #5698 / #5778), which says nothing about the web user whose resolver serves the door. ⛔ **Not a fault, and the remedy is the opposite of one** — do not re-provision on this alone. The `checks[]` finding names which read was refused. | `sudo bridge:check` (re-run as the account that can read it) |
