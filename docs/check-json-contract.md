...
| `bridge_side_unverified` | A block is present and **this run could not measure** this bridge's half: (ssh) the pinned-line probe returned `unvalidated` — an `authorized_keys` file it needed was not readable by this run (typically because it was not root), or `AuthorizedKeysFile` named an entry this run could not resolve (`%U` with no uid lookup), and the pinned line may be in exactly that file (card#8976); (http) the bearer token file could not be seen or read by **this process** (cards #5698 / #5778), which says nothing about the web user whose resolver serves the door. ⛔ **Not a fault, and the remedy is the opposite of one** — do not re-provision on this alone. The `checks[]` finding names which read was refused. ⚠ **The command is the remedy for the COMMON cause, not for every cause** (DL-359): the two ssh arms that need a root-resolved `AuthorizedKeysFile` are reachable only from a run that was ALREADY root, and a privileged re-run supplies neither a missing `posix` extension (the `%U` cause) nor a read that root itself was refused — for those, act on the cause the `checks[]` finding names, not on this command. | `sudo bridge:check` (re-run as the account that can read it) |
