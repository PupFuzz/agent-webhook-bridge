...
| **Advice is WITHHELD** — a derived `next_steps` entry the document would otherwise carry is suppressed because another finding in the SAME document contradicts the advice it gives | **No** | **Read `next_steps` as advice that is CURRENT, never as a census of states.** The array is DERIVED from findings the document already carries, so an entry's absence never meant the underlying STATE was absent — the finding it derives from is still in `checks[]`, which is where a consumer keying on state must read. What narrows is which advice the document offers: no entry gains or loses a field, no `state` value is added or removed, no severity is invented, and the direction is strictly *fewer instructions that would MUTE a failure reported two lines above*. ⚠ **This row covers withholding an ENTRY and nothing else.** Narrowing what a `state` value MEANS where a consumer reads state, rather than narrowing when advice is offered, changes a key's meaning and bumps. **First applied: DL-360**, where the `no_block` entry is withheld for an agent whose recorded block is reported LOST (§7a). |
| A **new `state` value** joins a `next_steps` entry | **No** | **Treat an unknown `state` as advice you do not understand, and read `checks[]`.** The vocabulary is open by the same reasoning that makes an added key additive: no shipped value changes meaning, no key moves, and the array is DERIVED from findings the document already carries — so a consumer that ignores an entry it cannot classify is never blinded to the underlying condition, which is in `checks[]` and, when it is a fault that gates, in `ok` and the exit code too. ⛔ **This row does NOT cover redefining an existing value**, which changes a key's meaning and bumps. **First applied: DL-368** (card#9150), which adds `github_webhook_missing` — an entry whose fault is a `fail` in `checks[]`, so a consumer ignoring the entry still sees `ok: false`. **Applied again: DL-399** (card#9717), which adds `github_webhook_other_hooks_only` beside it; the paragraph below states why that also NARROWS `github_webhook_missing`'s population without redefining its meaning. |

**`schema` is currently `1`.** It stayed `1` through DL-251 (stage 10), which re-assigned 21 findings from `warn` to `unvalidated` and added three: both rows above apply independently, and either alone is sufficient. It stayed `1` again through DL-255 (card#5698), which ADDED the top-level `agent_scope_coverage` key (§3a) and re-assigned three more findings to `unvalidated` — the added-key row and the pre-disclosed-precision row, again independently sufficient. It stays `1` through DL-259 (card#5698), the fail/ok severity audit, on the pre-disclosed-precision row alone: three findings move to `unvalidated` and one `ok` splits into `ok`/`warn`, adding no key and inventing no severity. Note for consumers keying on `severity`: like DL-258 before it, this moves findings OUT of `ok`, so a consumer treating `ok` as "everything this leg covers is fine" gets *fewer* false all-clears and no new shape. It stays `1` through DL-265 (card#5796) on the corrected-verdict row: one install state — a `secret_dir` that exists and is not a directory — moves `warn` → `fail`, adding no key and inventing no severity, and moving in that same direction. It stays `1` through DL-266 (card#5596) on the **added-key** row alone: `inventory.undeclared_silent` (§6) joins the document and nothing else moves — the four `disposition` values are untouched, no severity is invented, and no existing key changes type or meaning. The declaredness the new key exposes rides **alongside** the disposition exactly as `not_run_reasons` does, which is what keeps that true by construction rather than by inspection. It stays `1` through DL-352 (card#8959) on the **added-key** row alone: the top-level `next_steps` array (§7a) joins the document and nothing else moves — no severity is invented, no `disposition` value is added, no existing key changes type or meaning, and the array is DERIVED from findings the document already carried rather than from a new measurement. The `state` vocabulary shipped in that entry has **four** values (`no_block`, `bridge_side_unverified`, `bridge_side_incomplete`, `seat_side_unreported`); the split of the middle two happened in review, before any tag carried the key, so no consumer ever saw a three-value vocabulary. **DL-359 (card#8976) does not move it either, and is recorded here because §7a's `state` DEFINITIONS are contract even though the prose around them is not:** no key is added, no severity and no `state` value is invented, no existing key changes type or meaning, and the emitted document is **byte-identical for every install shape** — nothing in the renderer was touched. What changed is this document's account of WHEN an already-shipped `state` fires: `bridge_side_unverified`'s ssh cause list did not cover the arm where the configured `ssh_account` was never LOOKED UP, and it read as exhaustive when the probe's arm set lives in code. Judged a **clarification of a documented meaning, not a change to one** — so no row of the table above is spent, and a consumer keying on `state` has nothing to re-write. It stays `1` through DL-360 (card#8973) on **TWO rows, one per half of the change, because neither half is covered by the other's**. The **added-key** row carries the first half: `board_tools.lost` joins `checks[]` as one more registered check, and an ADDED LEG is not a corrected verdict — no existing leg's classification of any install state moves, no severity is invented, no `disposition` value is added, and no existing key changes type or meaning. ⚠ **That row does NOT reach the second half.** The `no_block` `next_steps` entry is now WITHHELD for an agent that leg reports LOST (§7a) — a change to an entry the document already emitted, which the added-key row says nothing about — so it rides on the **withheld-advice** row, on the terms that row states: the state itself is unchanged and still readable in `checks[]`, and `NextStepState` gains no member. It stays `1` through **DL-364 (card#8974)** on the **pre-disclosed-precision** row alone, exactly as DL-259 did: `board_tools.client_half` gains a third severity it could not emit before — its `ok` REPORTED line splits into `ok`/`warn` on whether the seat's reported channel-server version is older than the one this bridge bundles. No key is added, **no severity is invented** (`warn` has been in the vocabulary since the beginning), no `disposition` value is added, no existing key changes type or meaning, and the exit code is untouched (only `fail` moves it). ⚠ **Note for consumers keying on `severity`:** this moves findings OUT of `ok` in the same direction the earlier splits did — a consumer treating `ok` as *"everything this leg covers is fine"* gets one fewer false all-clear, and no new shape. ⚠ **`next_steps` MOVES WITH IT AND IS NOT A SECOND ROW:** the `warn` is the SAME finding as the report, so `seat_side_unreported` continues to key on whether a call was OBSERVED — a stale seat has reported and still owes nothing here. That is the definition below unchanged, not a new one. It stays `1` through **DL-399 (card#9717)** on the **new-`state`-value** row alone: `github_webhook_other_hooks_only` joins the `next_steps` vocabulary, no key is added or moved, no severity is invented and none moves (both causes are the `fail` `github_webhook_missing`'s was), and no existing key changes type. ⚠ **The question that row's ⛔ raises is answered rather than assumed: `github_webhook_missing`'s POPULATION narrows, and its MEANING does not.** Every entry still carrying it satisfies that value's shipped definition word for word — *no webhook on the repo delivering to this install's receiver, established by reading the whole list* — and the entries that moved out of it now carry a value a `schema: 1` consumer is already instructed to treat as advice it does not understand, with the condition itself unchanged in `checks[]` and still `ok: false`. A consumer keying on `github_webhook_missing` to mean *go add a webhook* therefore stops being handed the one population on which that advice was WRONG: the direction is the same *fewer false instructions* the corrected-verdict and pre-disclosed-precision rows above travel in, and no consumer branch written against `schema: 1` becomes incorrect. ⛔ **What WOULD have bumped** is keeping one value and redefining it — teaching `github_webhook_missing` to mean something a consumer's existing branch would act on wrongly — which is precisely why this change spends a new value instead.
...
| `agent` | string \| null | The agent a per-agent check ran for; `null` for a global check. **This attribution exists only in this document** — the text report has no per-check framing to put it in. ⚠ **A RUN-ONCE check has no agent even when its finding is ABOUT one**, and `board_tools.lost` (DL-360) is the case that makes that bite: its population is the agents this install has RECORDED, which is not a subset of the agents the per-agent loop iterates — the seats it exists to report are exactly the ones with no config to iterate — so every one of its findings carries `agent: null` and names the seat in `message` prose only. A consumer that needs the seat name for a lost block must parse the message, which §2 says is not a contract; there is deliberately no structured carrier for it. |
...
```jsonc
"inventory": {
  // ⛔ THE COUNTS ARE DERIVED FIGURES AND ARE DELIBERATELY NOT WRITTEN HERE. This block
  // states the SHAPE; `CheckRunner::inventory()` owns the numbers and the registered set
  // grows, so every numeral printed here was true at one commit and false a few later —
  // and a reader who cannot see the program acts on the stale one. Re-derive with
  // `php artisan bridge:check --format=json | jq .inventory`; the committed
  // `tests/Fixtures/check-golden/minimal.txt` states a baseline install's line, and
  // `CheckCommandRegistrationTest` pins the registered set itself.
  "registered": <int>,
  ...
  "not_requested": <int>,
  "not_run": <int>,
  "not_run_reasons": [<string>, ...],   // DISTINCT reasons; one envelope can account for several checks
  "unexplained_not_run": [],
  "undeclared_silent": []
}
```
...
| `undeclared_silent` | array of check id | Checks that ran, reported nothing, and never **declared** that silence (DL-266). **A real defect, not a degraded message** — the asymmetry with the row above is the point: a missing not-run reason costs a sentence about a check the run already accounted for, while an undeclared silence is the state stage 8 could not tell apart from a bug (the check falling off the end of its generator through a path its author did not intend). Every path in the registry was audited and declared when the mechanism landed, so a non-empty array means a silent path was **added** without that judgement being made. Normally empty; a non-empty array is a bridge bug worth reporting, not an install problem. **It does not claim the silence is wrong** — a declaration records that the author *intended* it, never that it is correct, so this is a floor on the unjudged paths and not a ceiling on the wrong ones. |
...
- **Every count is keyed by check id — with `undeclared_silent` the one stated exception.** A per-agent check is **one** row however many agents it ran for — one that ran for two of three agents counts once, as having run, and nothing here scopes that to the agents it reached. Use `checks[].findings[].agent` for per-agent detail, and note that a check can be `silent` for one agent and `reported` for another while the row says `reported`. `undeclared_silent` is recorded per **execution** rather than folded out of the disposition, precisely because that per-agent case is the one a per-id fold hides: a check that reports for agent A and is undeclared-silent for agent B ends the run `reported`, so deriving the list by filtering `disposition === "silent"` would drop it. It is still a list of check **ids** — an id appears at most once however many of its executions were undeclared, because it names the unjudged path, not the size of the roster.
...
One entry per outstanding item, in config order, each naming the ONE command to run next. ⚠ **The unit is NOT uniformly the agent:** a board-tools entry is per AGENT, and a `github_webhook_missing` or `github_delivery_silent` entry is per **(agent, scope)** — one repo's missing hook or silent record concerns every agent subscribed to it, and each is told separately (`NextSteps::webhookSteps()`). It is the machine half of the operator report's **NEXT STEPS** block; both are rendered from the same value, so the command a consumer runs is the command the report printed.

**Two subjects since DL-368 (card#9150):** the board-tools `state` values are about **board-tools** enablement (DL-217); `github_webhook_missing` (DL-368) and `github_delivery_silent` (DL-382) are about a declared **github subscription** — one whose repo carries no webhook delivering here, and one whose own delivery record has gone quiet. The block is *this install is not wired end to end*, which both answer to.

**`scope`** (added in DL-368, on §2's added-key row) is the subscription scope an entry is about. It is **always present** and is `null` on every board-tools state — a consumer telling an absent key from a null would be handling two shapes for one field. It is non-null only on the github states, whose remedy is not an instruction until you know which repo it names.

**The definitions below are carried here for the consumer; their OWNER is `app/Bridge/Check/NextStepState.php`'s docblock**, and every other surface (the runbook callout, the changelog) points at this section rather than restating them. First match wins, in this order.

| `state` | Meaning | `command` |
| --- | --- | --- |
| `no_block` | The agent's YAML carries no `board_tools:` block at all **AND this install has no record of one for that agent**. Since DL-360 the absent block is necessary and no longer sufficient: an agent whose recorded block is gone is a `board_tools.lost` **fail** instead, and gets no entry here — this state's premise is that it is a QUESTION the operator may sit on forever, and its own answer (*set `enabled: false`*) would MUTE that failure. | `bridge:provision-tools --agent=<name>` (prints a paste-ready skeleton; it never edits YAML) |
| `bridge_side_unverified` | A block is present and **this run could not measure** this bridge's half: **(ssh)** the pinned-line probe returned `unvalidated` — a severity SEVERAL causes reach, not one: an `authorized_keys` file it needed was not readable by this run (typically because it was not root); or `AuthorizedKeysFile` named an entry this run could not resolve (`%U` with no uid lookup), so the pinned line may be in exactly that file; or the configured `board_tools.ssh_account` was never LOOKED UP at all, because this PHP process has no `posix_getpwnam` — nothing was named, nothing was attempted, and whether that account exists is UNKNOWN (DL-259). ⚠ **Those are EXAMPLES, not a closed set** (card#8976): the probe's arms live in code and it may grow one, so key on `state` and read the `checks[]` finding for what THIS run could not consult — never on a cause list. **(http)** the bearer token file could not be seen or read by **this process** (cards #5698 / #5778), which says nothing about the web user whose resolver serves the door. ⛔ **Not a fault, and the remedy is the opposite of one** — do not re-provision on this alone. The `checks[]` finding names what could not be consulted — a file, an `AuthorizedKeysFile` entry, or the account itself. ⚠ **The command is the remedy for the COMMON cause, not for every cause** (DL-359): the ssh arms that need a root-resolved `AuthorizedKeysFile` are reachable only from a run that was ALREADY root, and a privileged re-run supplies neither a missing `posix` extension (the `%U` and never-looked-up causes) nor a read that root itself was refused — for those, act on the cause the `checks[]` finding names, not on this command. | `sudo bridge:check` (re-run as the account that can read it) |
| `bridge_side_incomplete` | A block is present and this bridge's half was **measured and found not usable** — a default-on block that could not satisfy itself, an http bearer that resolved absent / blank / insecure / shared with a sibling, or an ssh pinned line that is absent, ambiguous, or grants what the forced command must deny. The specific fault is a `checks[]` finding with its own cause and cure. | `bridge:provision-tools --agent=<name>` (transport-aware) |
...
| `github_webhook_missing` | A github subscription this agent declares has **no webhook on the repo delivering to this install's receiver**, and this run established that by READING the repo's whole hook list — **and since DL-399 (card#9717), the repo carries NO webhooks at all.** A repo carrying webhooks of which none delivers here is `github_webhook_other_hooks_only` instead: the population narrows, every word of the meaning holds. `scope` names the repo. ⛔ **A MEASURED absence and never a blind read** — a run that could not enumerate the repo's hooks (no token, a 401/403/404, a network failure, a 200 that is not a hook list) reports `unvalidated` in `checks[]` and gets **no entry here**, because instructing an operator to add a webhook off an unmeasured read sends them to re-create one that is already there. ⚠ **The only state whose fault is a `fail`**, so a document carrying this entry also carries `ok: false`. ⚠ **The `command` is a RE-ASK, not the remedy** — a github webhook lives in the repo's own settings and `bridge:provision` manages the kanban provider only, so no command on the bridge box can create it. | `bridge:check` (after someone with `admin:repo_hook` has added the hook) |
| `github_webhook_other_hooks_only` | The same MEASURED absence one step more precise (DL-399, card#9717): none of the repo's webhooks delivers to this install's receiver, **and the repo carries other webhooks**. `scope` names the repo. ⛔ **The next action is a QUESTION, which is why it is a state and not a rewording** — is this repo already served by ANOTHER install's bridge? **If it is**, the stale thing is THIS install's declaration and the fix is to drop that scope's subscription; **if it is not**, this install's own hook is gone and someone with `admin:repo_hook` adds it. Doing the second on the first points a **second card-mover** at one board, where two movers race on every PR event — so a consumer must not read this state as *add a webhook*. ⛔ **It is not a claim that another install serves the repo**, and cannot be: the leg counts the repo's hooks and never reads their delivery URLs out of the box (that list is the whole fleet's), so what it reports is *hooks are there and none is ours*. ⚠ **Its fault is a `fail`, exactly like `github_webhook_missing`** — the cause and the remedy differ, the verdict does not — so a document carrying this entry also carries `ok: false`. ⚠ **The `command` is a RE-ASK**: no command on the bridge box can create a repo webhook, and none can answer the question either. | `bridge:check` (after someone has established which situation it is, and acted) |
| `github_delivery_silent` | A github subscription this agent declares has gone quiet **by its own delivery record**: this install's receiver has recorded no delivery for that scope at all, or none within the silence threshold derived from the scope's own gaps between deliveries (DL-382). `scope` names the repo. ⛔ **An inference from SILENCE, not a measured fault** — a genuinely quiet repo produces it too, so its `checks[]` finding is a `warn` and the entry moves neither `ok` nor the exit code. ⛔ **It witnesses the DELIVERY side only**: deliveries that arrive and are dropped before any agent wakes read as healthy, so the ABSENCE of this entry is not evidence an agent is being woken. ⚠ **Never beside `github_webhook_missing` for the same scope** — where this run read the hook list and found the hook gone, that entry carries the cause. A record this run could not read, or could not derive a threshold from while the silence is inside the floor, is `unvalidated` in `checks[]` and gets **no entry here**. | `bridge:check` (after someone who can see the repo's webhook settings has looked) |

**Why `unverified` and `incomplete` are two states and not one with a hedge:** `state` is a machine contract, and the two take **opposite** remedies. A consumer (or an operator reading the text) that treated *could not read* as *broken* would re-provision a correctly wired install — a privileged remediation window spent on nothing, which is card#7756's exact cost one surface over. Both inputs carry unmeasurable members (`CheckCommand::severityMeansSetupIncomplete()` folds the pinned-line probe's `unvalidated` into *incomplete* for the DL-225 advisory; `BoardToolAgentResolver` excludes a token it could not see from its index), so the derivation reads the SEVERITY of each leg's finding rather than its presence in a set.

**Bounds:**

- **It is a POINTER TO WORK, never an inventory of the install.** Its population is the agents whose YAML **parsed and was read to the end** — an agent that failed to load, or aborted at the classifier gate, is absent here while being present in `findings_outside_registry` / `checks[]` — and an agent that deliberately opted out (`board_tools.enabled: false`) is absent because it owes nothing. **An empty list means nothing is outstanding.** ⚠ **A FOURTH ABSENCE since DL-360:** an agent whose RECORDED block is gone is absent here too — it is a `board_tools.lost` fail in `checks[]`, and the remedy is on that line rather than in this block.
- **The http `bridge_side_unverified` arm has no fixture in the suite.** The ssh arm is exercised (a non-root probe environment); the http one needs a bearer file this process cannot read, which the suite cannot stage portably. Both reach the same `match` arm in `NextSteps::stateOf()`, so the code path is shared — the untested part is the resolver's per-agent recording on that path, stated here rather than rounded up.
- **`seat_side_unreported` is keyed on OBSERVATION, not on the finding's severity being `ok` (DL-364).** The client-half report reads `warn` when the seat's reported channel-server version is older than the bundled one — the SAME finding, saying the seat called AND that its snapshot is behind — so such an agent has been observed and owes nothing in this block; its remedy (re-deploy the snapshot) is on the `checks[]` line, not here. ⛔ A consumer must not re-derive this list by filtering `checks[]` for `board_tools.client_half` findings that are `ok`: that reproduces the pre-DL-364 defect and tells an operator to go ask a seat to call when it already has.
...
- **It cannot move `ok` or the exit code** — the derivation yields values, not findings, and only a `fail` finding moves the verdict. ⚠ **That is a statement about the DERIVATION, not about every fault it points at (DL-368).** A `github_webhook_missing` entry points at a leg that DOES fail, so a document carrying one also carries `ok: false` — because of the finding in `checks[]`, never because of the entry.
...
| The `github_webhook_missing` state, the three-way severity split behind it, and the fleet-leak control (DL-368); the `github_webhook_other_hooks_only` split, its count and the ask (DL-399) | `tests/Feature/Console/Check/GitHubWebhookSubscriptionCheckTest.php` |
| The `github_delivery_silent` state and every severity behind it (DL-382); the threshold derivation, replayed over the incident fixture | `tests/Feature/Console/Check/GitHubDeliveryHistoryCheckTest.php`; `tests/Unit/Bridge/Check/DeliveryHistory/` |
