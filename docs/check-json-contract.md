...
⛔ **AND THEY ARE NOT SANITISED — a `message` can carry bytes a foreign principal wrote, verbatim, control characters included** (card#9121, DL-366). A finding may interpolate a string this install did not author, and several already do: the channel bind-FAILURE marker's detail (written by whatever connector snapshot that seat runs, on a path the bridge does not own), a deployed channel-server `package.json`'s `version` and the resolved deployment path (both chosen by the OS account being inspected), a remote host's `stderr`/stdout/`error` string and the key-algorithm token from that account's own `authorized_keys` (the `--probe-tools-ssh` leg), and a board-tools responder's `error` string or raw body snippet. ⚠ **That list is what a REVIEW found, not a bound the document can enforce** — a call site that interpolates a foreign string is a producer whether or not anyone noticed, so read it as illustration and treat every `message` as untrusted. The **terminal** report escapes and length-caps those spans; **this document deliberately does not**, because these bytes are a write contract and changing them would move every consumer's payload at once with no version to key on. ⚠ **A consumer that renders a `message` to a terminal, a log viewer, a web page or a shell owns that encoding at its own boundary** — treat every `message` as untrusted text, not as prose this install vouches for.
