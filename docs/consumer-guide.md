...
>
> 1. **Live wake = the MCP channel push** (`notifications/claude/channel`, via the channel server in `examples/channel-servers/` over a unix socket). This is the only genuinely **event-based** path — the bridge pushes the instant it processes the webhook. It's *best-effort*: a push to a disconnected session reaches no transport at all, and for one that lands while the session is mid-turn **nothing on this path establishes whether the notification is dropped or merely surfaced at that session's next turn boundary** (card#9172/DL-370). So it must be paired with the recovery layer — which is the right answer under either.
...
> **⚠ A byte-offset cursor is NOT safe across retention (changed in DL-199).** Retention now runs automatically off the inbound webhook (`bridge.retention.*`, **on by default**) and **rewrites `inbox*.jsonl` whole-file** to drop aged lines — so the file can SHRINK under a tail consumer, and a byte offset into it then points at the wrong place. This was already possible whenever an operator ran `bridge:prune`; DL-199 makes it the default. **Track position by intent `id`, not by byte offset** (`bridge:inbox`'s own seen-cursor already does exactly this, and is pruned in step with the file). If you must tail by offset, detect truncation (stat the file; if it shrank, re-read from the top and de-dup by `id`) or set `BRIDGE_RETENTION_ENABLED=false` and prune on your own schedule.
>
> **⏱ Recovering a missed dispatch is bounded by a SECOND, SHORTER window — read this before you rely on it.** `php artisan bridge:replay <N>` consults `webhook_events` (the authoritative audit store, never touched by inbox operations) and re-runs dispatch — **but only while that event still has its payload.** Retention has two independent windows: `older_than` (**30d** by default) deletes the event ROW, and `null_payloads_older_than` (**7d** by default, and **ON by default since DL-315** — it shipped OFF before that) nulls the PAYLOAD while keeping the row. **Replay REFUSES a payload-nulled event and exits 1**; it cannot reconstruct the payload, and dispatching an empty one in its place would stage a fabricated intent into your inbox. So **the operative recovery window is the payload one, not the 30d row one**, and a row you can still see in `bridge:inspect` is not necessarily replayable — `bridge:stats` counts the unrecoverable ones separately as `errored (NOT replayable …)`. Both windows are per-install operator settings, so **ask your bridge operator what yours are** rather than assuming the defaults; the knobs and the upgrade note are in [`CLAUDE_DEPLOYMENT.md`](../CLAUDE_DEPLOYMENT.md) § *Retention config (DL-199)*. ⇒ **Do not design a consumer whose recovery story is "replay it later"** past that window — track position by intent `id` and re-read, per the paragraph above.
...
> **Both legs are load-bearing — wire SessionStart AND PreToolUse.** They cover different windows, and the live `channel_push` is best-effort (for an intent pushed while the session is mid-turn, nothing on the live path reports whether it surfaced — the inbox is what recovers it either way):
